...
module.exports = {
  ...
  treeForApp(tree) {
    let extensions = this._enabledExtensions;
    let thisDir;
    let extensionsTree;
    ...
    extensionsTree = new Funnel(path.join(thisDir, 'vendor', 'initializers'), {
      include: extensions.map((name) => `${name}.js`),
      destDir: 'initializers'
    });

    return mergeTrees([extensionsTree, tree]);
  },
